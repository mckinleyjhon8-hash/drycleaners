# Pricing Model — The Garment Concierge
## Per-order concierge pricing + optional membership

**Last updated:** 2026-06-14
**Status:** Working model. Trade rates are assumptions pending negotiation (see §9).

---

## 1. Philosophy

1. **Pay-per-item is the core.** Dry-cleaning demand is lumpy and irregular, so
   per-order pricing self-matches demand and proves a margin on *every* order.
2. **Membership is a loyalty layer, not bundled cleaning.** A low monthly fee buys
   perks (free delivery, members' pricing, priority) — it is near-pure margin and
   drives frequency. We do **not** sell all-you-can-eat bundles (usage risk + churn).
3. **We compete on experience + aggregation, not price.** Fluff & Tumble already
   offers cheap direct collection/delivery (free over £15). We win the affluent,
   time-poor segment with a premium experience and by routing each item to the best
   specialist cleaner — something a customer can't easily do themselves.

---

## 2. Revenue streams

| Stream | Description |
|---|---|
| Cleaning markup | Spread between **trade rate** (what we pay cleaners) and **concierge price** (what we charge). Primary engine. |
| Membership fees | ~£19/mo, near-pure margin. |
| Express surcharge | Premium for next-day turnaround. |
| Corporate / Airbnb contracts | Later — recurring B2B volume. |

---

## 3. Sourcing strategy — two anchor partners

Your competitor comparison shows no single cleaner wins everything. Route by category:

| Category | Best partner | Why (from comparison data) |
|---|---|---|
| Suits, shirts, outerwear, bedding, everyday | **Fluff & Tumble** | Cheapest across most standard items; free delivery £15; 24–48h |
| Leather, suede, bridal, UGG, handbags, specialist | **One Stop** | Cheapest on leather/suede/wedding/UGG/designer items |

**Target trade rate:** 25–35% off the cleaner's retail for committed weekly volume
(we bring demand + handle marketing/logistics). Model below uses a conservative **30%**.

---

## 4. Concierge price list & margins (illustrative)

Trade cost = cleaner retail − 30% (assumed). Concierge price = the customer-facing
price; **the margin is the buffer we keep on every item**, before membership and
delivery fees. Everyday items source from Fluff & Tumble, specialist from One Stop. (Prices below
are shown rounded; the live site displays them as charm prices ending in .99 —
e.g. £23.99 — which changes each margin by ≤1p.)

### Business & Formalwear
| Item | Cleaner retail | Trade cost | Concierge price | Our margin |
|---|---|---|---|---|
| Two-piece suit | £14.95 | £10.47 | £24 | £13.53 (56%) |
| Three-piece suit | £17.95 | £12.57 | £29 | £16.43 (57%) |
| Jacket / blazer | £8.50 | £5.95 | £14 | £8.05 (58%) |
| Trousers | £7.50 | £5.25 | £12 | £6.75 (56%) |
| Waistcoat | £5.50 | £3.85 | £9 | £5.15 (57%) |
| Shirt, washed & pressed | £2.95 | £2.07 | £4.50 | £2.43 (54%) |
| Tie or scarf | £4.50 | £3.15 | £7 | £3.85 (55%) |
| Standard dress | £12.00 | £8.40 | £20 | £11.60 (58%) |
| Silk dress | £14.50 | £10.15 | £24 | £13.85 (58%) |
| Dress & jacket | £18.50 | £12.95 | £30 | £17.05 (57%) |
| Skirt | £7.50 | £5.25 | £12 | £6.75 (56%) |
| Pleated skirt | £9.50 | £6.65 | £15 | £8.35 (56%) |

### Outerwear, Knitwear & Occasion
| Item | Cleaner retail | Trade cost | Concierge price | Our margin |
|---|---|---|---|---|
| Short coat | £11.50 | £8.05 | £19 | £10.95 (58%) |
| Long coat | £14.00 | £9.80 | £24 | £14.20 (59%) |
| Puffer jacket | £16.00 | £11.20 | £27 | £15.80 (59%) |
| Canada Goose / Moncler | £35.00 | £24.50 | £55 | £30.50 (55%) |
| Wax jacket | £45.00 | £31.50 | £69 | £37.50 (54%) |
| Jumper / cardigan | £6.50 | £4.55 | £11 | £6.45 (59%) |
| Hoodie / sweatshirt | £7.00 | £4.90 | £12 | £7.10 (59%) |
| Evening gown | £16.75 | £11.73 | £28 | £16.27 (58%) |
| Wedding dress | £75.00 | £52.50 | £120 | £67.50 (56%) |
| Bridesmaid dress | £25.00 | £17.50 | £42 | £24.50 (58%) |
| Saree | £25.00 | £17.50 | £42 | £24.50 (58%) |
| Asian two-piece | £9.95 | £6.97 | £18 | £11.03 (61%) |

### Specialist, Home & Laundry
| Item | Cleaner retail | Trade cost | Concierge price | Our margin |
|---|---|---|---|---|
| Leather jacket | £30.00 | £21.00 | £48 | £27.00 (56%) |
| Leather coat | £40.00 | £28.00 | £65 | £37.00 (57%) |
| Leather trousers | £15.00 | £10.50 | £29 | £18.50 (64%) |
| Motorbike leathers | £50.00 | £35.00 | £85 | £50.00 (59%) |
| UGG boots | £15.00 | £10.50 | £24 | £13.50 (56%) |
| Designer handbag | £15.00 | £10.50 | £28 | £17.50 (62%) |
| Duvet (single) | £16.00 | £11.20 | £26 | £14.80 (57%) |
| Duvet (double / king) | £19.50 | £13.65 | £32 | £18.35 (57%) |
| Pillows (each) | £10.50 | £7.35 | £17 | £9.65 (57%) |
| Curtains (per pair) | £7.50 | £5.25 | £13 | £7.75 (60%) |
| Service wash (per kg) | £4.50 | £3.15 | £8 | £4.85 (61%) |
| Trouser hem | £9.50 | £6.65 | £15 | £8.35 (56%) |
| Zip replacement | £11.00 | £7.70 | £18 | £10.30 (57%) |

**Every customer price carries a 54–65% gross margin over the trade rate** — and that's
*before* the £19/mo membership and delivery fees, which are additional buffer. Per-order
logistics (collection + delivery) come out of this margin (see §6).

---

## 5. Membership — "The Concierge Membership"

**£19 / month.** Benefits:
- Free collection & delivery on every order (no minimum)
- 15% off all cleaning
- Priority scheduling + express slots
- A dedicated garment-care concierge

**Why it works:**
- The £19 fee is **near-pure margin** (we're not pre-committing cleaning).
- The 15% discount is funded by the 50%+ markup — a member order still nets ~40%+.
- It removes the "am I using it enough?" anxiety that kills bundle subscriptions.
- Even at **one order/month** a member is profitable, and members order *more*
  because collection is friction-free.

**Why not all-you-can-eat bundles:** heavy users cost more than the fee (we lose
money); light users churn. Worst fit for lumpy demand.

---

## 6. Per-order unit economics

**Typical pay-as-you-go basket:** 1 suit + 3 shirts + 1 dress.

| Line | Amount |
|---|---|
| Customer pays (concierge) | £24 + 3×£4.50 + £20 = **£57.50** |
| We pay cleaner (trade) | £10.47 + 3×£2.07 + £8.40 = **£25.08** |
| **Gross spread** | **£32.42 (56%)** |
| − Collection & delivery (batched) | ~£6.00 |
| − Packaging | ~£1.50 |
| − Card processing (~2%) | ~£1.15 |
| **Net contribution** | **~£23.77 (≈41%)** |

The single biggest swing factor is **route density** — batching collections cuts the
per-order logistics cost. Concentrate Tier-1 postcodes before widening coverage.

---

## 7. Delivery fees

| Plan | Collection & delivery |
|---|---|
| Pay As You Go | From £4.99; free over £30 |
| Concierge Member | Always free |

(Modest PAYG fee protects margin on small orders; membership removes it as a perk.)

---

## 8. Pricing guardrails

- Maintain **≥45% gross margin per item** after trade rate.
- The premium must be justified by **experience + aggregation**, never gouging.
- Re-price when trade rates, fuel, or competitor prices move materially.
- Keep specialist items (leather/bridal) priced for value — they're infrequent and
  high-trust; don't over-inflate.

---

## 9. Key assumptions & risks

| Assumption / risk | Action |
|---|---|
| **30% trade rate is assumed, not agreed** | Negotiate with Fluff & Tumble + One Stop using the comparison data as leverage |
| **Affluent segment will pay the premium** | Test with first 20 customers; track repeat rate |
| **Route density needed for logistics margin** | Launch concentrated in Tier-1 postcodes (Broughton, Woburn Sands, Tattenhoe…) |
| **Customers could go direct to F&T** | Lead on aggregation + experience, not price; never advertise as "cheaper" |

---

## 10. Open decisions (to revisit with data)

- Membership price point: £15 vs £19 (A/B once live).
- Free-delivery threshold for PAYG (£25 vs £30).
- Express surcharge level (e.g. +£X or +25%).
- Whether to add a high-touch VIP tier later (£49/mo, dedicated driver windows).
